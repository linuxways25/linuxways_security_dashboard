#!/usr/bin/env python3
"""
Written ✍🏻 by Atif Ahmed - Platform Architect & Arch Linux Enthusiat 
...
Run:
    python3 linuxways_security_dashboard.py
...
def listening_ports():
    """Find listening TCP/UDP sockets and resolve application/PID/user."""
    rows, seen = [], set()
    ...
    # Fallback if the OS restricts psutil socket ownership information.
    if not rows and cmd_exists("ss"):
        out = run_cmd(["ss", "-H", "-lntup"], 5)
        for line in out.splitlines():
            ...
            app, pid, user = "unknown", "", ""
            m = re.search(r'users:\(\("([^"]+)"(?:,pid=(\d+))?', line)
            if m:
                app, pid = m.group(1), m.group(2) or ""
            if pid:
                try:
    ...
    rows.sort(key=lambda r: int(r[0]) if r[0].isdigit() else 999999)
    return rows[:12]



def firewall_state():
    """Use UFW only for firewall status."""
    if not cmd_exists("ufw"):
        return "UFW NOT INSTALLED"
    out = run_cmd(["ufw", "status"], 4)
    if "Status: active" in out:
        return "ACTIVE"
    if "Status: inactive" in out:
        return "INACTIVE"
    return "UNKNOWN" if out else "UFW ERROR"
...
def score(d):
    ...
    if d["firewall"] == "UFW NOT INSTALLED":
        s -= 12
    elif d["firewall"] in ("INACTIVE", "UFW ERROR", "UNKNOWN"):
        s -= 15
...
class SecurityDashboard:
    ...
    def build(self):
        ...
        # Ports
        card = self.box()
        card.pack_start(self.section("LISTENING PORTS • APPLICATIONS"), False, False, 0)
    ...
    def render(self):
        ...
        self.set_color(
            self.sec["firewall"],
            "good" if d["firewall"] == "ACTIVE" else "bad"
        )
